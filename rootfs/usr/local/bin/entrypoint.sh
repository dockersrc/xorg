...
# @@Created          :  Friday, Jun 26, 2026 15:29 EDT
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for xorg
...
#SERVICE_USER="${SERVICE_USER:-xorg}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-xorg}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    if [ "$ENTRYPOINT_MESSAGE" = "yes" ]; then
      echo "Executing entrypoint script for xorg"
    fi
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
